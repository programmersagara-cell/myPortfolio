  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        status.classList.add('err'); status.textContent = '[!] ' + (data.error || 'Something went wrong.');
        if (data.csrf_token) $('#csrfToken').value = data.csrf_token;
      }
    } catch (err) {
      status.classList.add('err');
      status.textContent = '[!] PHP backend unavailable on this host.';
    } finally {
      $('#cfSubmit').disabled = false;
    }
  });

  // Fetch a CSRF token when PHP is available
  fetch('api/contact.php?action=token')
    .then(r => r.ok ? r.json() : null)
    .then(d => { if (d && d.token) $('#csrfToken').value = d.token; })
    .catch(() => {});
})();
