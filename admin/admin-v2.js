...
const addDefaults = {
  ...
  certifications: { category: '', name: '', issuer: '', year: '', image: '' }
};
...
simpleEditor._fields = function (listSel, fields) { this._f = this._f || {}; if (fields) this._f[listSel] = fields; return this._f[listSel]; };
async function uploadListItemImage(listSel, arr, i, file) {
  const fd = new FormData(); fd.append('image', file);
  const r = await api('../api/upload.php', { method: 'POST', body: fd });
  arr[i].image = r.url;
  simpleEditor(listSel, simpleEditor._fields(listSel), arr);
}
function simpleEditor(listSel, fields, arr) {
  simpleEditor._fields(listSel, fields);
  const wrap = $(listSel); wrap.innerHTML = '';
  if (!arr.length) wrap.innerHTML = '<p style="color:var(--muted)">Empty - click "Add entry".</p>';
  arr.forEach((item, i) => {
    const div = document.createElement('div');
    div.className = 'item';
    let html = '<div class="head"><h3>#' + (i + 1) + '</h3><button type="button" class="btn danger sm">Remove</button></div>' +
      fields.map(f => '<label>' + f[0] + '</label><input type="text" data-k="' + f[1] + '" value="' + esc(item[f[1]] || '') + '">').join('');
    // Optional image field (type 'image') with preview + upload
    const imgField = fields.find(f => f[2] === 'image');
    if (imgField) {
      html += '<label>' + esc(imgField[0]) + '</label>' +
        '<div class="shots" data-certimg></div>' +
        '<div class="row"><button type="button" class="btn secondary sm" data-upimg>🖼️ Upload image</button>' +
        '<input type="file" accept="image/jpeg,image/png,image/webp" hidden data-imgfile>' +
        '<span class="status mono" data-imgstatus style="margin:0"></span></div>';
    }
    div.innerHTML = html;
    if (imgField) {
      const imgWrap = div.querySelector('[data-certimg]');
      const src = item[imgField[1]];
      imgWrap.innerHTML = src
        ? '<div class="shot"><img src="' + esc(src) + '" alt="certificate image"><button type="button" title="Remove" data-rmimg>✕</button></div>'
        : '<span style="color:var(--muted);font-size:.8rem">No image yet — click "Upload image".</span>';
      imgWrap.querySelector('[data-rmimg]')?.addEventListener('click', () => { arr[i][imgField[1]] = ''; simpleEditor(listSel, fields, arr); });
      div.querySelector('[data-upimg]').addEventListener('click', () => div.querySelector('[data-imgfile]').click());
      div.querySelector('[data-imgfile]').addEventListener('change', async e => {
        const file = e.target.files[0];
        e.target.value = '';
        if (!file) return;
        const st = div.querySelector('[data-imgstatus]');
        st.textContent = 'Uploading ' + file.name + '...';
        try { await uploadListItemImage(listSel, arr, i, file); st.textContent = '[OK] Image uploaded — remember to Save.'; }
...
function loadAll() {
  fetch('../api/content.php?format=json', { headers: { 'Cache-Control': 'no-store' } })
    ...
    .then(saved => {
      ...
      simpleEditor('#certList', [['Category', 'category'], ['Certification name', 'name'], ['Issuer', 'issuer'], ['Year', 'year'], ['Certificate image', 'image', 'image']], certArr);
    })
